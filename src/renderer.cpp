...
void Renderer::Render(Shooter const &shooter, EnemyShipManager const &enemyShipManager) {
    SDL_Rect block;
    ...
    // Render enemy ships
    SDL_SetRenderDrawColor(sdl_renderer, 0xFF, 0xFF, 0xFF, 0xFF);
    for (std::vector<EnemyShip*> thisCol : enemyShipManager.ships) {
        for (EnemyShip* thisShip : thisCol) {
            for (SDL_Point const &point : thisShip->body) {
                block.x = point.x * block.w;
                block.y = point.y * block.h;
                SDL_RenderFillRect(sdl_renderer, &block);
            }
